...
aspect HelloAspect {
	...
	//unlock pointcut for synchronized block
		before(Object l): (unlock()) && args(l) && instrumentationScope()
		{		
			synchronized (HelloAspect.class) 
			{
	...
	pointcut traceMethod(): (execution (* *.*(..)) || call(* *.*(..)) ||threadRealatecall()|| constructionCall()  || traceThrowingExceptionCall()) && !javaPackageRealtedCall() && !threadRealatecall() &&!(execution(synchronized* *.*(..))) &&  instrumentationScope() ;
	before(): traceMethod() 
	{
	 ...
	 */
	   ...
	   private static synchronized void updateLockInfo(StaticPart thisJoinPointStaticPart, Object l) {
			...
			log.log(Level.INFO," Synchronized block Lock: FieldName "+ fieldName+" Trace method ThreadId "+ Thread.currentThread().getId()+" \n");
	   ...
	   private static synchronized void updateUnlockInfo(StaticPart thisJoinPointStaticPart, Object l) {
			...
			thread = getThreadId(Thread.currentThread());
			fieldName=getFieldNamelock(l.toString(),className);
			log.log(Level.INFO," Synchronized block Unlock: FieldName "+fieldName+" Trace method ThreadId "+ Thread.currentThread().getId()+" \n");
