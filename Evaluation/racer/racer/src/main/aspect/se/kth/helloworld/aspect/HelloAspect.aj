...
aspect HelloAspect {
	 ...
	 */
	   ...
	   private static synchronized void updateLockInfo(StaticPart thisJoinPointStaticPart, Object l) {
			...
			//log.log(Level.INFO,"Trace Synchronized lock block: "+thisJoinPointStaticPart.getKind()+" Field Name "+fieldName+" Source Location: "+thisJoinPointStaticPart.getSignature()+" CalsName"+className+" Trace ThreadId "+ Thread.currentThread().getId()+" \n");
	   ...
	   private static synchronized void updateUnlockInfo(StaticPart thisJoinPointStaticPart, Object l) {
			...
			thread = getThreadId(Thread.currentThread());
			fieldName=getFieldNamelock(l.toString(),className);
			//log.log(Level.INFO,"Trace  Unlock block: "+thisJoinPointStaticPart.getKind()+" Field Name "+fieldName+" Source Location: "+thisJoinPointStaticPart.getSignature()+" CalsName"+className+" Trace ThreadId "+ Thread.currentThread().getId()+" \n");
	   ...
	   private static synchronized void updateReentrantLockInfo(StaticPart thisJoinPointStaticPart,Object l) {
			...
			//log.log(Level.INFO,"Trace ReentrantLock lock block: "+thisJoinPointStaticPart.getKind()+ "Field Name"+fieldName+" Source Location: "+sourceLocation+" CalsName"+className+" Trace ThreadId "+ Thread.currentThread().getId()+" \n");
	   ...
	   private static synchronized void updateReentrantUnlockInfo(StaticPart thisJoinPointStaticPart, Object l) {
			...
			//System.out.println("AspectJ ClassName: " +className+"Method Name "+methodName+"\n");
			//log.log(Level.INFO,"Trace ReentrantLock unlock block: "+thisJoinPointStaticPart.getKind()+ "Field Name"+fieldName+" Source Location: "+sourceLocation+" CalsName"+className+" Trace ThreadId "+ Thread.currentThread().getId()+" \n");
	   ...
	   private static synchronized void updateMethodInfo(StaticPart thisJoinPointStaticPart, boolean val) {
			...
			if(thisJoinPointStaticPart.getSignature().getName().length() > 0  && ((prevMethod != thisJoinPointStaticPart.getSignature().getName()) && (thisJoinPointStaticPart.getKind().toString() == "method-execution")))
			{
	...
	if(thisJoinPointStaticPart.getKind().toString() == "method-call")
	{	
	   ...
	   private static synchronized  void tracethreadstartjoin(StaticPart thisJoinPointStaticPart, Thread childThread) {
			...
			fieldName="";
				...
				//log.log(Level.INFO,"Trace Thread method methodName "+ methodName+" pointcut:" + thisJoinPointStaticPart.getKind()+" Trace method ThreadId "+ Thread.currentThread().getId()+" \n");
				updateGlobalVar(sign,className,methodName,thread,sourceLocation,lineNo,fieldName,threadaspectj,packagename);
				global.createInvokeInstruction();
