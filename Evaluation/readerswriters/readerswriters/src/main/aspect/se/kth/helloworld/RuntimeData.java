		   ...
		   try
			{
			   ...
			   File file = new File(location);
				...
				if(!isFileExist)
				{
					...
					location = "readerswriters/src/main/java/se/kth/helloworld/RWVSNDriver.java";
...
public void displayErrorTrace() {	
		...
		se.kth.jpf_visual.ErrorTracePanel gui = new se.kth.jpf_visual.ErrorTracePanel();
			Path p= new se.kth.tracedata.jvm.Path(app,stack);
			 /*RWVSNDriver.startVisTime = System.nanoTime();
			    long totaltime =RWVSNDriver.startVisTime-RWVSNDriver.startMainTime;
			    System.out.println("before time:"+TimeUnit.NANOSECONDS.toMillis(totaltime)+"\n");*/
				gui.drowJVMErrTrace(p, true);	
				/* RWVSNDriver.endTraceTime = System.nanoTime();
				    long totaltimeafter =RWVSNDriver.endTraceTime-RWVSNDriver.startMainTime;
				    System.out.println("After time:"+TimeUnit.NANOSECONDS.toMillis(totaltimeafter)+"\n");*/
