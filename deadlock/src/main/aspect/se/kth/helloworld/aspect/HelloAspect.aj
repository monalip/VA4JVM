...
aspect HelloAspect {
	...
	pointcut synchPoint(): execution(synchronized* *.*(..)) && !within(HelloAspect) && !within(RuntimeData); // or call()}*/
	 ...
	 * thread inforamtion
	 * */
	after(Thread childThread) : call(* Thread+.start()) && !within(HelloAspect) && !within(RuntimeData) && target(childThread)
	{
	...
	pointcut traceMethod(): (execution (* *.*(..)) || (execution(*.new(..))) || call(* Thread+.start()) || call(* Thread+.join()) || call(* Thread+.wait()) || call(* java..*.*(..)) || call(* java.lang.Object.*(..)))  && !(synchPoint()) && !within(RuntimeData) && !cflow(within(HelloAspect)) ;
	after(): traceMethod() 
	{
